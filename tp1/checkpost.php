...
/* Vérification de l'URL CodeAcademy  en vérifiant qu'il s'agit bien d'une URL et qu'il contient https://www.codecademy.com 
 * et qu'il est suivi de /fr/pseudo
 */
if (isset($_POST['linkCodeAcademy']) && preg_match('/^(https:\/\/www.codecademy.com\/)[a-z]{2,3}[\/]{1}[\w]{4,20}/i', $_POST['linkCodeAcademy']) && filter_var($_POST['linkCodeAcademy'], FILTER_VALIDATE_URL))
{
    $textURL = '';
    $checkUrl = true;
}
...
{
    $textURL = 'ce n est pas une URL';
    $checkUrl = false;
}
//Vérification du format du numéro de téléphone saisi
if (isset($_POST['phone']) && preg_match('/^[0][1-79](([-.])?([0-9][0-9])){4}$/', $_POST['phone']))
{
    $textPhone = '';
    $checkPhone = true;
}
...
{
    $textPhone = 'Le numéro n\'est pas valide, recommencez !';
    $checkPhone = false;
